...
- **`clean(resources)`**: 빌드 시작 전, 현재 빌드 대상 리소스에 해당하는 파일/디렉터리만 선택적으로 삭제합니다. Command/Agent는 해당 파일만, Skill은 서브디렉터리 전체를 삭제하며, 전역 파일(`GEMINI.md`, `CLAUDE.md`, `AGENTS.md`)은 항상 삭제됩니다. 빌드 대상이 아닌 기존 파일은 출력 디렉터리에 그대로 유지됩니다.
- **`clean_all()`**: `--clean` 옵션 사용 시 호출됩니다. `commands/`, `agents/`, `skills/`, `.codex/`, `prompts/` 디렉터리와 전역 파일(`GEMINI.md`, `CLAUDE.md`, `AGENTS.md`) 전체를 삭제합니다.
...
3.  **정리 및 배포**: `full_clean` 여부에 따라 `emitter.clean_all()`(전체 삭제) 또는 `emitter.clean(&resources)`(선택적 삭제)를 호출한 후, `emitter.emit`으로 변환된 결과물과 추가 파일(`extras`)을 파일 시스템에 기록합니다.
