...
- `build` 명령어를 구현하고, 실행 시 `agb.yaml` 파일을 찾아 로드하는 기본 파이프라인을 연결합니다.
...
- **Build 인자:**
  - `--config` (또는 `-c`): 설정 파일 경로를 지정합니다. 기본값은 `tests/fixtures/agb.yaml`로 설정하세요.
...
- **테스트 케이스 2:** `agb build` 실행 시 기본 경로(`tests/fixtures/agb.yaml`)를 사용하여 로드 성공 메시지가 출력되는지 확인.
...
2. **기본 빌드 명령 테스트:** `tests/fixtures/agb.yaml`이 존재하는 환경에서 `agb build` 호출 시 성공 결과가 반환되는가?
