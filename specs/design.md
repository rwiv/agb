...
`syncer::patcher::MdPatcher`는 원본 파일의 구조를 유지하면서 특정 필드를 업데이트하기 위해 다음 방식을 사용합니다:

- **YAML 기반 프론트매터 업데이트**: `serde_yaml`을 사용하여 프론트매터를 파싱하고 필요한 필드(`description`)를 수정한 뒤 다시 직렬화합니다. 이 과정에서 필드 순서가 변경되거나 주석이 유실될 수 있으나, 멀티라인 데이터를 포함한 모든 유효한 YAML 구조를 안전하게 처리할 수 있습니다.
