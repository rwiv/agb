...
본 문서는 `agb` 시스템의 데이터 처리 규칙, 운영 제약 사항 및 타겟별 변환 명세를 정의합니다. 파일 구조 및 설정 파일 규격에 대해서는 [format.md](./format.md)를 참조하십시오.

## 1. 메타데이터 처리 규격

### 1.1 병합 우선순위 (Merge Priority)
