...
## Phase 1: 모델 이동 및 의존성 역전 (Model Migration)
*   **Task 1.1: `TransformedFile` 이동 및 `transformers` 의존성 제거**
    *   *성공 기준:* `TransformedFile`이 `resource` 모듈로 이동하고, `transformers` 모듈은 더 이상 `emitter`를 알 필요가 없게 된다.
*   **Task 1.2: 프로젝트 전반의 임포트 경로 업데이트 및 검증**
    *   *성공 기준:* 변경된 모델 위치에 따라 `builder`, `transformers`, `emitter`의 코드가 수정되고 정상적으로 빌드된다.

## Phase 2: Emitter 모듈 통합 (Module Integration)
