...
이 문서는 `agb` 프로젝트의 전체 개발 여정을 관리합니다. 각 테스크는 독립적인 `docs/tasks/TASK-X-Y.md` 문서에 상세 가이드를 가집니다.
...
*   **Task 1.2: `agb.yaml` 파싱 및 설정 모델링**
    *   *성공 기준:* 프로젝트 루트의 `agb.yaml` 파일을 읽어 Rust 데이터 구조체로 정확히 역직렬화한다.
...
*   **Task 2.3: 리소스(Markdown/JSON) 로딩 및 유효성 검사**
