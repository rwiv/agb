...
## 목표
- `map.yaml`을 통한 타겟별 메타데이터 자동 매핑 기능 구현.
...
- [ ] `map.yaml` 파일을 파싱하기 위한 로직 구현 (serde 활용).
...
- [ ] `src/loader/mod.rs`: `ResourceLoader`가 소스 루트에서 `map.yaml`을 로드하도록 수정.
...
## Phase 4: 검증 및 테스트
- [ ] 통합 테스트(`tests/`)에 `map.yaml` 매핑 케이스 추가.
