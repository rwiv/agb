...
- 사용자는 빌드 결과물이 생성될 디렉터리에 `agb.yaml` 설정 파일을 작성합니다.
- `agb.yaml`에는 소스 리소스(Plugins, AGENTS.md)가 위치한 절대 경로(`source`)를 반드시 명시해야 합니다.
- 사용자는 `agb.yaml`에 포함할 플러그인의 특정 리소스를 명시적으로 선언합니다.
- 빌드 시 지정된 에이전트 규격(Target)에 맞춰 결과물을 `agb.yaml`이 위치한 경로에 생성합니다.
